...
export class YoutubeService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (!this.baseUrl) {
      throw new Error('YOUTUBE_BASE_URL no está configurada en las variables de entorno');
    }
  ...
  async searchVideos(
    ...
    try {
      this.logger.log(`🔍 Buscando en YouTube: "${query}" (max: ${maxResults}, región: ${regionCode})`);
      
      const searchResponse = await this.makeSearchRequest(query, maxResults, regionCode);
      ...
      const results = this.formatSearchResults(searchResponse.data, videosResponse.data);
      
      this.logger.log(`✅ Búsqueda exitosa: ${results.length} videos encontrados`);
      return results;
  ...
  private formatSearchResults(searchData: any, videosData: any): YouTubeSearchResult[] {
    ...
    return searchData.items.map(item => {
      ...
      return {
        ...
        duration: this.parseDuration(videoDetails?.contentDetails?.duration || 'PT0S'),
