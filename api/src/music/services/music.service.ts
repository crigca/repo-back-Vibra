...
export class MusicService {
  ...
  async searchSongs(searchDto: SearchSongsDto): Promise<YouTubeSearchResult[]> {
    this.logger.log(`🔍 Buscando canciones: "${searchDto.query}" (max: ${searchDto.maxResults})`);
    ...
    try {
      ...
      this.logger.log(`✅ Búsqueda completada: ${results.length} canciones encontradas`);
      return results;
  ...
  async createSong(createSongDto: CreateSongDto): Promise<Song> {
    this.logger.log(`💾 Guardando canción: "${createSongDto.title}" por ${createSongDto.artist}`);
    ...
    if (existingSong) {
      this.logger.warn(`⚠️ Canción ya existe con youtubeId: ${createSongDto.youtubeId}`);
    ...
    try {
      const songData = {
        ...createSongDto,
        publishedAt: createSongDto.publishedAt ? new Date(createSongDto.publishedAt) : undefined
      };
      ...
      this.logger.log(`✅ Canción guardada exitosamente con ID: ${savedSong.id}`);
  ...
  async findSongById(id: string): Promise<Song> {
    ...
    this.logger.log(`✅ Canción encontrada: "${song.title}" por ${song.artist}`);
    return song;
  }
  ...
  async getAllSongs(limit: number = 50, offset: number = 0): Promise<Song[]> {
    this.logger.log(`📋 Obteniendo canciones (limit: ${limit}, offset: ${offset})`);
