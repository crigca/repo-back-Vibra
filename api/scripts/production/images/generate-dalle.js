/**
 * Script de generación de imágenes con DALL-E 3
 *
 * Genera 50 imágenes aleatorias distribuidas por tiers:
 * - Tier 1: 20 imágenes
 * - Tier 2: 15 imágenes
 * - Tier 3: 10 imágenes
 * - Tier 4: 5 imágenes
...
async function generateWithDallE(promptText, genre) {
  ...
  return {
    ...
    generator: 'DALL-E-3',
    metadata: { aiModel: 'dall-e-3', revisedPrompt: response.data.data[0].revised_prompt },
  };
}
...
async function generateImage(genre) {
  ...
  // Generar con DALL-E
  const aiResult = await generateWithDallE(promptText, genre);
  ...
  try {
    ...
    for (const [tierKey, imageCount] of Object.entries(IMAGES_PER_TIER)) {
      ...
      for (let i = 0; i < selectedGenres.length; i++) {
        ...
        try {
          console.log(`   [${i + 1}/${imageCount}] Generando "${genre}" con DALL-E...`);
    ...
    const costDalle = stats.success * 0.04;
    ...
    console.log(`💰 Costo total: $${costDalle.toFixed(2)} USD`);
    console.log('='.repeat(70));
