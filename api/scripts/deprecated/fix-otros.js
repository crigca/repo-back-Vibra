...
async function getSinCategoriaSongs() {
  try {
    ...
    const otros = allSongs.filter(s => s.genre === 'Otros');

    stats.total = otros.length;

    console.log(`📊 Encontradas ${otros.length} canciones "Otros"\n`);
    return otros;
...
async function main() {
  ...
  console.log('las canciones marcadas como "Otros" usando el');
