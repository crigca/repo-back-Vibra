...
async function deleteRiiverdanceSong() {
  try {
    ...
    const songs = await AppDataSource.query(
      `SELECT id, title, artist, "youtubeId" FROM songs WHERE title ILIKE '%RIIVERDANCE%' OR title ILIKE '%RIVERDANCE%'`
    );

    console.log(`\n📊 Encontradas ${songs.length} canción(es):\n`);

    if (songs.length === 0) {
      console.log('❌ No se encontró ninguna canción con "RIIVERDANCE"');
